...
let jwksCache:{expires:number;keys:Jwk[]}|null=null;

async function getJwks(url:string){
  if(jwksCache && jwksCache.expires>Date.now()) return jwksCache.keys;
  const r=await fetch(url); if(!r.ok) throw new Error(`JWKS_HTTP_${r.status}`);
  const b:any=await r.json();
  jwksCache={expires:Date.now()+5*60_000,keys:Array.isArray(b.keys)?b.keys:[]};
...
async function verifyJwt(token:string):Promise<any>{
  ...
  const keys=await getJwks(config.auth.jwksUrl);
  const jwk=keys.find(k=>k.kid===x.h.kid && k.kty==='RSA'); if(!jwk) throw new Error('JWT_KEY_NOT_FOUND');
...
export async function authenticate(req:FastifyRequest):Promise<User>{
  ...
  const driverId=String(p.driver_id??p.driverId??p.sub??''); if(!driverId) throw new Error('DRIVER_ID_MISSING');
  return {sub:String(p.sub),driverId};
}

export class RedisRateLimiter{
  private redis?:Redis;
  constructor(){if(config.redisUrl)this.redis=new Redis(config.redisUrl,{lazyConnect:false});}
...
export function registerV13(app:FastifyInstance){
  ...
  app.addHook('onRequest',async(req,reply)=>{
    if(req.url.startsWith('/health')||req.url.startsWith('/v1/exchanges/trans-eu/callback')||req.url.startsWith('/v1/webhooks/trans-eu')) return;
    try{(req as any).lfUser=await authenticate(req)}catch(e){return reply.code(401).send({error:'unauthenticated',detail:String(e)})}
  });
  ...
  app.post<{Body:{lat:number;lon:number;accuracy?:number}}>( '/v13/driver/location',async(req,reply)=>{
    ...
    if(repo) await repo.savePosition(u.driverId,b.lat,b.lon,b.accuracy);
